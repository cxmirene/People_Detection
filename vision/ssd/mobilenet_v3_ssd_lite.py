...
from .ssd import SSD, GraphPath_mb3
...
def create_mobilenetv3_ssd_lite(moudle, num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    base_net = MobileNetV3(moudle, num_classes).features
    # print(base_net)

    if moudle=="Small":
        source_layer_indexes = [
            GraphPath_mb3(11, 'conv'),
            20,
        ]
        header = 96
    else:
        source_layer_indexes = [
            GraphPath_mb3(14, 'conv'),
            24,
        ]
        header = 112
    ...
    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(header * width_mult), out_channels=6 * 4,
    ...
    classification_headers = ModuleList([
        SeperableConv2d(in_channels=round(header * width_mult), out_channels=6 * num_classes, kernel_size=3, padding=1),
    ...
    return SSD(num_classes, base_net, source_layer_indexes,
               extras, classification_headers, regression_headers, is_test=is_test, config=config, use_mb3=True)
